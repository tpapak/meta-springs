//! Capacity-scaling max-flow on top of the spring/PCG primitive.
//!
//! Outer schedule (Edmonds-Karp scaling style):
//!
//!   Δ = 2^floor(log2(c_max))
//!   f = 0  (explicit signed flow per undirected edge)
//!   F = 0
//!   while Δ ≥ 1:
//!     repeat:
//!       active = edges with symmetric residual `c − |f| ≥ Δ`
//!       (these are edges that could carry at least Δ more units in either direction)
//!       if active is empty or s,t disconnected: break
//!       solve electrical flow on active sub-graph, unit demand s→t,
//!         pinned sink (no εI), conductance = residual
//!       d = edge currents from φ
//!       verify div_s(d) ≈ 1 (else sub-graph disconnected, break)
//!       α = max scaling such that |f + α·d| ≤ c per edge
//!       push = α · div_s(d)
//!       if push < Δ: break this scale (no Δ-augmenting flow remains)
//!       f += α·d
//!       F += push
//!     Δ /= 2
//!
//! Validated against OR-Tools.
...
use gc_rust::lap_solver::{CsrLap, Jacobi, Preconditioner, TwoLevelAmg};
use gc_rust::lap_solver::pcg::pcg_solve as lib_pcg_solve;
...
fn gen_graph(n: usize, target_e: usize, seed: u64, cap_max: i64) -> Graph {
    ...
    let cap_hi = (cap_max as u64) + 1;
    for k in 1..n {
        let u = perm[k];
        let parent = perm[rng.gen_range(0, k as u64) as usize];
        let cap = (rng.gen_range(1, cap_hi)) as i64;
...
/// PCG with vertex pinning AND any user-supplied preconditioner.
/// After each preconditioner apply, the pinned coordinate of the
/// preconditioned residual is zeroed so the search direction stays in
/// the pinned subspace.
fn pcg_pinned_with<P: Preconditioner>(
    csr: &CsrLap,
    b: &[f64],
    pin: usize,
    precond: &P,
...
fn capscale_maxflow(g: &Graph, max_per_scale: usize, use_amg: bool) -> (f64, usize, usize, f64) {
    let n = g.n;
    let m = g.edges.len();
    ...
    let mut total_aug = 0_usize;

    let c_max: f64 = caps.iter().cloned().fold(0.0, f64::max);
    let mut scale_log = c_max.log2().ceil() as i32;
    ...
    let t0 = Instant::now();
    while scale_log >= 0 {
        let delta = 2.0_f64.powi(scale_log);
        // Inner loop: keep augmenting at this scale while we can push ≥ Δ units.
        for _ in 0..max_per_scale {
            // Active edges: residual ≥ Δ in either direction
            let active: Vec<bool> = (0..m).map(|i| caps[i] - f[i].abs() >= delta).collect();
            let n_active: usize = active.iter().filter(|&&a| a).count();
            if n_active == 0 { break; }

            // Build active sub-Laplacian with conductance = residual
            let weighted: Vec<(u32, u32, f64)> = g.edges.iter().enumerate()
                .filter_map(|(i, &(u, v, _))| if active[i] {
                    let residual = (caps[i] - f[i].abs()).max(1e-12);
                    Some((u as u32, v as u32, residual))
                } else { None })
                .collect();
            if weighted.is_empty() { break; }
            let csr = CsrLap::from_canonical_weights(&weighted, n);
            let (phi, pcg_it) = if use_amg {
                let amg = TwoLevelAmg::new(&csr, 1e-12, 2);
                pcg_pinned_with(&csr, &b, pin, &amg, pcg_tol, max_pcg_iter)
            } else {
                pcg_pinned(&csr, &b, pin, pcg_tol, max_pcg_iter)
            };
            total_pcg += pcg_it;

            // Compute direction d_e and div_s; verify s,t connected.
            ...
            // Disconnected sub-graph detection: PCG can't realise unit demand.
            if (div_s - 1.0).abs() > 1e-3 { break; }

            // α: largest feasible scaling.
            ...
            let push = alpha * div_s;
            // Stop at this scale when can't push at least Δ units.
            if push < delta { break; }
    ...
    (big_f, total_aug, total_pcg, t0.elapsed().as_secs_f64() * 1000.0)
}
...
fn main() {
    ...
    let max_per_scale: usize = args.iter().find_map(|a| a.strip_prefix("--max-per-scale=")?.parse().ok()).unwrap_or(1000);
    let use_amg: bool = args.iter().any(|a| a == "--amg");

    if let Some(path) = dimacs {
        let pb = PathBuf::from(&path);
        let g = parse_dimacs(&pb).expect("parse dimacs");
        let (f_or, us_or) = run_ortools(driver, &pb).expect("ortools");
        let (f_cs, aug, pcg, ms) = capscale_maxflow(&g, max_per_scale, use_amg);
        let f_or = f_or as f64;
        let abs_err = (f_cs - f_or).abs();
        let rel_err = abs_err / f_or.max(1.0);
        let name = pb.file_name().and_then(|s| s.to_str()).unwrap_or("").trim_end_matches(".max");
        println!("name,V,E,F_or,F_CS,abs_err,rel_err,augmentations,pcg_iters,cs_ms,or_ms");
        println!("{},{},{},{:.0},{:.2},{:.2},{:.5},{},{},{:.1},{:.2}",
            name, g.n, g.edges.len(), f_or, f_cs, abs_err, rel_err, aug, pcg, ms, us_or as f64 / 1000.0);
        return;
    }
    ...
    let cap_max: i64 = args.iter().find_map(|a| a.strip_prefix("--cap-max=")?.parse().ok()).unwrap_or(100);

    println!("# Cap-scaling max-flow. n={n}, m={m_target}, cap_max={cap_max}, max_per_scale={max_per_scale}");
    println!("seed,V,E,F_or,F_CS,abs_err,rel_err,augmentations,pcg_iters,cs_ms,or_ms");
    let mut max_rel_err = 0.0_f64;
    for seed in 0..n_seeds {
        let g = gen_graph(n, m_target, 0xDEAD_BEEF_C0DE_F00D ^ seed, cap_max);
        let tmp = std::env::temp_dir().join(format!("cs_{seed}.max"));
        ...
        println!("{},{},{},{:.0},{:.2},{:.2},{:.5},{},{},{:.1},{:.2}",
            seed, g.n, g.edges.len(), f_or, f_cs, abs_err, rel_err, aug, pcg, ms, us_or as f64 / 1000.0);
