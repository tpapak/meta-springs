#!/usr/bin/env python3
"""Multi-commodity max-throughput LP via CBC (COIN-OR) using PuLP.

Same problem as mc_lp_oracle.py but with CBC backend — the standard
open-source LP/MIP solver.

Input JSON: { n, edges, commodities }
Output JSON: { F_per, total, time_ms, solver }
"""
import json
import sys
import time
...
def main():
    ...
    if prob.status != 1:  # 1 = Optimal
        print(json.dumps({"error": f"CBC status {prob.status}", "time_ms": elapsed_ms}))
        sys.exit(1)
