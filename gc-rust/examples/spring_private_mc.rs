//! Per-commodity "private" stiffness multi-axis spring MC.
//!
//! Per-commodity Laplacian: instead of a single shared-residual L, each
//! commodity i sees its OWN edge conductance
//!     G_{i,e} = max(c_e − Σ_{j≠i} |f_j,e|, ε)
//! — the headroom available to commodity i, holding other commodities
//! fixed. Each PCG solve on L_i uses *that* private conductance, so K
//! commodities get K genuinely different electrical-flow directions
//! (rather than the single shared direction set used by `spring_joint_mc`).
//!
//! Round-robin step: pick i, build L_i, PCG, augment with sampled α_i.
...
fn spring_private_mc(
    ...
    for _it in 0..n_iters {
        ...
        // PRIVATE residual for commodity i:
        //   G_{i,e} = max(c_e − Σ_{j≠i} |f_j,e|, ε)
        // (NOT shared residual — i sees full c_e minus competitors only)
        let mut private_g = vec![0.0_f64; m];
        for e in 0..m {
            let mut other = 0.0;
            for j in 0..k { if j != i { other += f[j * m + e].abs(); } }
            private_g[e] = (caps[e] - other).max(1e-12);
        }
        let weighted: Vec<(u32, u32, f64)> = g.edges.iter().enumerate()
            .map(|(e, &(u, v, _))| (u as u32, v as u32, private_g[e]))
            .collect();
...
fn main() {
    ...
    let res = spring_private_mc(&g, n_iters, alpha_lo, alpha_hi, seed);
    println!("\n[Spring PRIVATE-stiffness MC]");
