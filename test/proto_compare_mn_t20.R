#!/usr/bin/env Rscript
# multinma fit on /tmp/t20.json with HalfNormal(0,1) prior on τ.
# Writes /tmp/mn_t20.json (same schema as proto_compare_mn.R).
...
write_json(list(
  dataset  = "/tmp/t20.json",
  prior    = "HalfNormal(0,1) on tau",
  mcmc     = list(iter = 3000, warmup = 1000, chains = 4, seed = 42),
  wall_sec = wall,
  tau2     = tau2_summary,
  effects  = effs
), "/tmp/mn_t20.json", auto_unbox = TRUE, digits = 6)

cat(sprintf("multinma  wall=%.2fs  tau2 mode=%.3f  median=%.3f  CI=(%.3f,%.3f)\n",
