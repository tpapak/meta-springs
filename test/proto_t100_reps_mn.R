#!/usr/bin/env Rscript
# multinma 30-rep bias sweep at T=100. Three priors per rep, summary stats only
# (no per-fit samples — those are too large for 30 reps × 3 priors).
...
for (r in seq_len(nReps)) {
  path <- sprintf("/tmp/t100_reps_bin/rep_%02d.json", r)
  d_raw <- fromJSON(path)
  d <- data.frame(
  ...
  for (pp in priors) {
    ...
    prior_results[[length(prior_results) + 1L]] <- list(
      ...
      tau2 = list(
        ...
        median = median(tau2), mean = mean(tau2),
  ...
  results[[length(results) + 1L]] <- list(
    rep = r, wall_sec = rep_wall, priors = prior_results
  )
}
t1_total <- Sys.time()
total <- as.numeric(difftime(t1_total, t0_total, units = "secs"))
cat(sprintf("\nmultinma T=100 30-rep total: %.1fs\n", total))
write_json(list(total_wall_sec = total, n_reps = nReps, results = results),
           "/tmp/t100_reps_bin_mn.json", auto_unbox = TRUE, digits = 6)
cat("Wrote /tmp/t100_reps_bin_mn.json\n")
