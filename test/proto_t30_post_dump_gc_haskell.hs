...
-- Haskell GC posterior dump for ONE T=30 multi-arm replicate under
-- HalfNormal(τ; σ=1).  Argv: <repIdx>.  Output /tmp/t30_post_gc_haskell.json.
module Main where
...
main = do
  args <- getArgs
  let rep = case args of [s] -> read s :: Int; _ -> 1
      path = printf "/tmp/t30_reps_bin/rep_%02d.json" rep
  Right studies <- readStudies path
  printf "T=30 multi-arm GC dump: rep %d (%d studies)\n" rep (length studies)
  case springGrandCanonicalBinAdaptive studies 100 of
    ...
    Right (_, gridFull) -> do
      ...
      BL.writeFile "/tmp/t30_post_gc_haskell.json" (A.encode json)
      printf "Wrote /tmp/t30_post_gc_haskell.json (%d grid pts × %d contrasts)\n"
