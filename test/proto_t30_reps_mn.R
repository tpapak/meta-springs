#!/usr/bin/env Rscript
# multinma sweep over /tmp/t30_reps_bin/rep_NN.json (NN = 01..30). Three priors
# per replicate: HalfNormal(τ; 0.5), HalfNormal(τ; 1), HalfCauchy(τ; 0.5).
# Records τ² posterior summary + per-contrast (1→j) mean+sd.
...
for (r in seq_len(nReps)) {
  path <- sprintf("/tmp/t30_reps_bin/rep_%02d.json", r)
  d_raw <- fromJSON(path)
  d <- data.frame(
  ...
  for (pp in priors) {
    ...
    for (j in seq_along(pn)) {
      x <- as.vector(sims[,,j])
      # parse "d[ref vs j]" → j label.  Multinma uses "d[ref vs trt]".
      lbl <- sub("^d\\[.* vs (.*)\\]$", "\\1", pn[j])
...
cat(sprintf("\nmultinma total wall: %.1fs (%d reps × 3 priors)\n", total, nReps))

write_json(list(total_wall_sec = total, n_reps = nReps, results = results),
           "/tmp/t30_reps_bin_mn.json", auto_unbox = TRUE, digits = 6)
cat("Wrote /tmp/t30_reps_bin_mn.json\n")
