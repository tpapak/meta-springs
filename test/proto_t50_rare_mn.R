#!/usr/bin/env Rscript
# multinma sweep for T=50 rare-events binary, 5 reps × 3 priors.
...
for (r in seq_len(nReps)) {
  path <- sprintf("/tmp/t50_rare_bin/rep_%02d.json", r)
  d_raw <- fromJSON(path)
  d <- data.frame(
...
write_json(list(total_wall_sec = total, n_reps = nReps, results = results),
           "/tmp/t50_rare_mn.json", auto_unbox = TRUE, digits = 6)
cat("Wrote /tmp/t50_rare_mn.json\n")
